...
def transform(records: list[dict]) -> pd.DataFrame:
    ...
    df["claim_url"] = df["claim_url"].apply(lambda v: v if isinstance(v, str) and v.strip() else None)

    df = ensure_column(df, "claim_embedding", None)
    df["claim_embedding"] = df["claim_embedding"].apply(lambda v: v if isinstance(v, list) else None)
