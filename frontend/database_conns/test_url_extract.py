...
import pytest
import logging
from database_conns.url_extract import extract_with_trafilatura, extract_with_firecrawl, extract_url
...
def test_extract_with_trafilatura_none(mocker, caplog):
    ...
    mocker.patch("database_conns.url_extract.trafilatura.fetch_url", return_value=None)
...
def test_extract_with_trafilatura_error(mocker, caplog):
    ...
    mocker.patch(
        "database_conns.url_extract.trafilatura.fetch_url",
...
def test_extract_with_firecrawl_valid(mocker, caplog):
    ...
    mocker.patch("database_conns.url_extract.Firecrawl", return_value=mock_firecrawl_app)
...
def test_extract_with_firecrawl_error(mocker, caplog):
    ...
    mocker.patch("database_conns.url_extract.Firecrawl", side_effect=Exception("Error"))
    mocker.patch.dict("database_conns.url_extract.environ", {"FIRECRAWL_API_KEY": "test_key"})
...
def test_extract_url_trafilatura_success(mocker, caplog):
    ...
    mock_trafilatura_fetch = mocker.patch(
        "database_conns.url_extract.trafilatura.fetch_url", return_value="test fetch")
    mock_trafilatura_extract = mocker.patch(
        "database_conns.url_extract.trafilatura.extract", return_value="test extract")
    mock_firecrawl = mocker.patch("database_conns.url_extract.extract_with_firecrawl")
