...
def _headline_metrics(df: pd.DataFrame) -> dict:
    ...
    days_to_resurface = (df["access_datetime"] - df["publish_datetime"]).dt.days
    return {
        "total_claims": len(df),
...
def _render_metric_row(metrics: dict):
    ...
    m1.metric(label="Total Claims Checked", value=f"{metrics['total_claims']:,}")
    ...
    m4.metric(label="Resurfaced Myths (>7d)", value=f"{metrics['resurfaced']:,}")
